...
export default function App() {
  ...
  const loadPdfs = async () => {
    try {
      ...
      console.warn('Error fetching PDFs:', err);
    }
  };
  ...
  const handleUploadFiles = async (fileList) => {
    ...
    addToast(`Uploading ${validFiles.length} file(s) to /pdf folder...`, 'info');
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        addToast(`Saved ${validFiles.length} note(s) into /pdf folder!`, 'success');
        await loadPdfs();
      } else {
  ...
  // Save rename to /api/rename
  const handleSaveRename = async (payload) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        addToast(`Renamed to "${payload.newName}" in /pdf folder!`, 'success');
        setEditingPdf(null);
        await loadPdfs();
      } else {
        addToast(data.error || 'Failed to rename file', 'error');
      }
    } catch (err) {
      console.error('Rename error:', err);
      addToast('Server error while renaming file', 'error');
    }
  };

  // Delete from /api/pdfs/:fileName
  const handleConfirmDelete = async (fileName) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        addToast(`Deleted "${fileName}" from /pdf folder`, 'info');
        setDeletingPdf(null);
        await loadPdfs();
      } else {
  ...
  const filteredPdfs = pdfs.filter(pdf => {
    ...
    if (searchQuery.trim()) {
      ...
      const matchTags = (pdf.tags || []).some(t => t.toLowerCase().includes(q) || `#${t.toLowerCase()}`.includes(q));
      ...
      <header className="fixed top-0 inset-x-0 z-40 bg-[#f9f9f7]/90 backdrop-blur-xl shadow-[0_1px_8px_rgba(0,0,0,0.04)] border-b border-stone-200">
        <div className="h-20 max-w-[1280px] mx-auto px-6 sm:px-10 flex items-center justify-between gap-4">
          <div className="flex items-center justify-between w-full gap-4">
            {/* Logo Brand */}
            <a className="flex items-center gap-2 group" href="#" onClick={() => { setSearchQuery(''); setActiveFilter('all'); }}>
        ...
        <div className="w-full max-w-[1280px] mx-auto px-6 sm:px-10 py-8 flex flex-col gap-8">
          ...
          {filteredPdfs.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-xs text-stone-500 max-w-sm mt-1">Try clearing your search query or select a different tag filter.</p>
              <button 
